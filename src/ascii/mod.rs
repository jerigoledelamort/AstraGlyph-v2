...
// --- Combined atlas ---
//
// The composite pass samples one texture, so the shading glyphs and the text
// font have to live in the same atlas. Layout: the `glyph_count()` shading
// glyphs first (indices unchanged, so every existing brightness->index mapping
// keeps working), then the `font5x7::CHAR_COUNT` printable-ASCII glyphs.
...
pub fn combined_glyph_count() -> usize {
    glyph_count() + font5x7::CHAR_COUNT
}

/// Build the combined atlas: shading glyphs followed by the text font, in the
/// same flat RGBA layout `build_atlas()` uses.
...
mod tests {
    ...
    #[test]
    fn combined_atlas_has_both_sections() {
        let atlas = build_combined_atlas();
        assert_eq!(combined_glyph_count(), glyph_count() + font5x7::CHAR_COUNT);
